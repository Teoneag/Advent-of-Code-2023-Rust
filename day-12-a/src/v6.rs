...
fn get_new_indexes(string: &str, nrs: &Vec<usize>, index: usize) -> HashMap<usize, u64> {
    ...
    for hash in hash_set {
        ...
        let nrs_sub = &nrs[index..index + i];

        if nrs_sub == consecutive_hashes {
...
fn get_nr_comb_and_index(
    ...
    for (key, value) in hash_map {
        let new_hash_map = get_nr_comb_and_index(springs, i_springs + 1, nrs, key);
        sum += value * new_hash_map;
    }
...
fn process_line(line: &str) -> u64 {
    ...
    get_nr_comb_and_index(&springs, 0, &nrs, 0)
}
